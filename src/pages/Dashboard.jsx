              ...
              <tbody>
                {allReminders.map((r, idx) => (
                  <tr key={r.id} style={idx % 2 === 0 ? styles.trEven : styles.trOdd}>
                    ...
                    <td style={{ ...styles.td, whiteSpace: 'nowrap' }}>
                      <button onClick={async () => { const reason = window.prompt('Reason for reassign / additional query for user:'); if (reason === null) return; if (!reason.trim() || reason.trim().length < 5) { alert('Please enter at least 5 characters'); return; } await axios.put(`/api/orders/reminders/${r.id}/reassign`, { reason: reason.toUpperCase() }); fetchAllReminders() }} style={{ padding: '3px 8px', background: '#f39c12', color: '#fff', border: 'none', borderRadius: '4px', fontSize: '9px', cursor: 'pointer', fontWeight: '600', marginRight: '4px' }}>Reassign</button>
