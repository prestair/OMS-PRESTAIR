...
function Dashboard() {
  ...
  // Restore scroll position after manual refresh
  useEffect(() => {
  ...
  const handleDeletedImport = (e) => {
    ...
    reader.onload = async (evt) => {
      ...
      const orders = data.map(row => ({
        ...
        deliveryAddress: row['Delivery Address'] || row['DELIVERY ADDRESS'] || '',
  ...
  const handleImport = (e) => {
    ...
    reader.onload = async (evt) => {
      ...
      const mapped = data.map(row => {
        ...
        orderNo: row['Order No'] || row['ORDER NO.'] || row['ORDER NO'] || '',
        status: row['DOD Status'] || row['Status'] || row['BL+REQ'] || '',
        deliveryDate: convertDate(row['Delivery Date']),
        deliveryRemarks: row['Delivery Remarks'] || row['Delivery Date Remarks'] || '',
        customerName: row['Customer Name'] || row['CUSTOMER NAME'] || '',
        gst: row['GST'] || '',
        billingAddress: row['Billing Address'] || row['BILLING ADDRESS'] || '',
        followUp: row['Follow Up'] || row['FOLLOW UP'] || '',
        salesRep: row['Sales Rep'] || row['SALES REP'] || '',
        deliveryAddress: row['Delivery Address'] || row['DELIVERY ADDRESS'] || '',
        phoneNo: String(row['Phone No'] || row['PHONE NO'] || ''),
        photography: row['Photography'] || '',
        photographyRemarks: row['Photography Remarks'] || '',
        siteVideo: row['Site Video'] || '',
        siteVideoRemarks: row['Site Video Remarks'] || '',
        review: row['Review'] || '',
        reviewRemarks: row['Review Remarks'] || '',
        siteVerification: row['Site Verification'] || '',
        siteVerificationRemarks: row['Site Verification Remarks'] || '',
        installationStatus: row['Installation Status'] || row['INSTALLATION STATUS'] || row['INSTALLATION\nSTATUS'] || '',
        installationRemarks: row['Installation Remarks'] || row['INSTALLATION REMARKS'] || row['INSTALLATION\nREMARKS'] || '',
        lop: row['LOP'] || '',
        sectionDrawing: row['Section Drawing'] || row['Section Drawing\nSD'] || row['Section Drawing SD'] || '',
  ...
  const getDailyFilteredData = () => {
    ...
    // Combined sort by FY (year) desc then order number desc — merges active + deleted together
    const getOrderParts = (orderNo) => {
      if (!orderNo) return { fy: 0, num: 0 }
      const parts = String(orderNo).split('/')
      let fy = 0, num = 0
  ...
  const getCellValue = (order, key) => {
    ...
    if (key === 'remarks') {
      return <span>{val || ''} {order.auditProofUrl && <><a href={order.auditProofUrl} target="_blank" rel="noreferrer" style={{ color: '#2980b9', fontSize: '10px', fontWeight: '700' }} onClick={e => e.stopPropagation()}>View Supporting</a>{isAdmin && <button onClick={async(e)=>{e.stopPropagation();if(window.confirm('Delete this supporting image?')){try{await axios.delete(`/api/delete-audit-proof/${order.id}`);fetchOrders()}catch{}}}} style={{marginLeft:'4px',padding:'1px 5px',background:'#e74c3c',color:'#fff',border:'none',borderRadius:'3px',fontSize:'9px',cursor:'pointer',fontWeight:'600'}}>Delete</button>}</>}</span>
    }
          ...
          <thead>
            <tr>
              ...
              {displayedColumns.map((col, colIdx) => {
                ...
                const colStyle = {...styles.th, whiteSpace:'normal', ...(isCenter?{textAlign:'center'}:{}), ...(col.key === 'client' && !userW ? {minWidth:'180px'} : {}), ...(col.key === 'poNo' && !userW ? {width:'70px', maxWidth:'70px'} : {}), ...(userW ? {width: userW+'px', minWidth: userW+'px'} : {})}
                return (
      ...
      {activeTab === 'daily' && (
        <div style={{ padding: '24px' }}>
          ...
          <div style={{ background: '#fff', borderRadius: '8px', padding: '6px 10px', marginBottom: '12px', boxShadow: '0 1px 4px rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', gap: '4px', flexWrap: 'wrap' }}>
            ...
            <button onClick={() => { setDailyFilter('percentReceived'); setDailyFilterValue([]); setDailyPercentMax(''); setDailyPercentDateFrom('') }} style={dailyFilter === 'percentReceived' ? styles.dailyBtnActive : styles.dailyBtn}>% Received</button>
            ...
            <button onClick={() => { setDailyFilter('orRecvd'); setDailyFilterValue([]); setOrTabSearch(''); setOrTabStatusFilter([]); setOrTabUserSearch('') }} style={dailyFilter === 'orRecvd' ? styles.dailyBtnActive : styles.dailyBtn}>OR Issue Report</button>
            ...
            <div style={{ ...styles.reportSection, marginBottom: 0, padding: '12px' }}>
              ...
              <div style={{ display: 'flex', gap: '6px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: '1', position: 'relative', minWidth: '100px' }}>
                  ...
                  {showUserDropdown && paperUserSearch && (<div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: '#fff', border: '1px solid #ddd', borderRadius: '4px', maxHeight: '120px', overflow: 'auto', zIndex: 50, boxShadow: '0 2px 6px rgba(0,0,0,0.1)' }}>{allUsers.filter(u => u.username !== user.username && ((u.fullName || '').toLowerCase().includes(paperUserSearch.toLowerCase()) || u.username.toLowerCase().includes(paperUserSearch.toLowerCase()))).map(u => (<div key={u.id} onMouseDown={e => e.preventDefault()} onClick={() => { setPaperIssueTo(u.username); setPaperUserSearch(u.fullName || u.username); setShowUserDropdown(false) }} style={{ padding: '4px 8px', cursor: 'pointer', fontSize: '10px', borderBottom: '1px solid #f0f0f0' }} onMouseEnter={e => e.currentTarget.style.background='#f0f8ff'} onMouseLeave={e => e.currentTarget.style.background='#fff'}>{u.fullName || u.username}</div>))}</div>)}
                </div>
            ...
            <div style={{ ...styles.reportSection, marginBottom: 0, padding: '12px' }}>
              ...
              <div style={{ display: 'flex', gap: '6px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: '1', position: 'relative', minWidth: '100px' }}>
                  ...
                  {showReturnUserDrop && returnUserSearch && (<div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: '#fff', border: '1px solid #ddd', borderRadius: '4px', maxHeight: '120px', overflow: 'auto', zIndex: 50, boxShadow: '0 2px 6px rgba(0,0,0,0.1)' }}>{allUsers.filter(u => u.username !== user.username && ((u.fullName || '').toLowerCase().includes(returnUserSearch.toLowerCase()) || u.username.toLowerCase().includes(returnUserSearch.toLowerCase()))).map(u => (<div key={u.id} onMouseDown={e => e.preventDefault()} onClick={() => { setReturnIssueTo(u.username); setReturnUserSearch(u.fullName || u.username); setShowReturnUserDrop(false) }} style={{ padding: '4px 8px', cursor: 'pointer', fontSize: '10px', borderBottom: '1px solid #f0f0f0' }} onMouseEnter={e => e.currentTarget.style.background='#f0f8ff'} onMouseLeave={e => e.currentTarget.style.background='#fff'}>{u.fullName || u.username}</div>))}</div>)}
                </div>
                <button onClick={async () => {
                  ...
                  <div key={idx} style={{padding:'8px 10px',marginBottom:'6px',background:ev.type==='RETURN'?'#eafaf1':'#f0f8ff',borderRadius:'6px',border:'1px solid #eee',borderLeft:`4px solid ${ev.type==='RETURN'?'#27ae60':'#2980b9'}`}}>
                    ...
                    {ev.type==='ISSUE' ? (
                      <div style={{fontSize:'10px',color:'#333',lineHeight:'1.6'}}>
                        <div>Issued by: <strong>{getFullName(ev.to)}</strong> → Requested by: <strong>{getFullName(ev.by)}</strong></div>
