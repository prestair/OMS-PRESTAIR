          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
            <div style={{ ...styles.reportSection, marginBottom: 0, padding: '12px' }}>
              ...
              <div style={{ display: 'flex', gap: '6px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
                ...
                <button onClick={async () => { if (!paperOrderNo.length || !paperIssueTo) { alert('Select Order & User'); return } const errors = []; const success = []; for (const on of paperOrderNo) { try { await axios.post('/api/orders/paper-requests', { orderNo: on, issueTo: paperIssueTo }); success.push(on) } catch (err) { errors.push(err.response?.data?.error || `${on}: Failed`) } } if (success.length > 0) { setPaperOrderNo(prev => prev.filter(o => !success.includes(o))); setPaperIssueTo(''); setPaperOrderSearch(''); setPaperUserSearch(''); fetchPaperRequests() } if (errors.length > 0) { alert('❌ Request Failed:\n\n' + errors.join('\n')) } else { setPaperOrderNo([]); alert('Submitted!') } }} style={{ padding: '6px 10px', background: '#1a1a2e', color: '#fff', border: 'none', borderRadius: '4px', fontSize: '10px', fontWeight: '600', cursor: 'pointer', whiteSpace: 'nowrap' }}>Issue</button>
              </div>)}
            </div>
            <div style={{ ...styles.reportSection, marginBottom: 0, padding: '12px' }}>
