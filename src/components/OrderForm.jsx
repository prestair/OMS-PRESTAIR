        ...
        <form onSubmit={handleSubmit} style={styles.form} onKeyDown={(e) => {
          ...
          <div style={{ padding: '10px', background: '#f0f8ff', borderRadius: '6px', border: '1px solid #bee5eb' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
              ...
              <input ref={proofInputRef} type="file" accept=".jpg,.jpeg,.png,.gif,.bmp,.webp,.tiff,.svg" onChange={(e) => { const f = e.target.files[0]; if (f) { if (f.size > 2 * 1024 * 1024) { setError('Image size must be less than 2MB'); setTimeout(() => setError(''), 3000); return } setProofFile(f); const ext = f.name.split('.').pop() || ''; const nameWithoutExt = f.name.replace(/\.[^/.]+$/, '').replace(/[^a-zA-Z0-9._-]/g, '_'); setProofFileName(nameWithoutExt + '.' + ext); setProofPreview(URL.createObjectURL(f)); setError('') } }} style={{ display: 'none' }} />
