...
router.post('/paper-requests', async (req, res) => {
  ...
  if (pendingReturn && pendingReturn.length > 0) return res.status(400).json({ error: `${(pendingReturn[0].requested_by||'').toUpperCase()}'s return request is pending for ${orderNo}. Please close it first.` })
...
router.post('/paper-requests/:id/accept', async (req, res) => {
  ...
  if (pendRet && pendRet.length > 0) return res.status(400).json({ error: `${(pendRet[0].requested_by||'').toUpperCase()}'s return request is pending for ${reqs[0].order_no}. Please close it first.` })
...
router.post('/paper-requests/reissue', async (req, res) => {
  ...
  if (!acceptedReqs || acceptedReqs.length === 0) return res.status(400).json({ error: 'No active issue found' })
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    if (req.body.akhilPoints !== undefined && req.user.role === 'admin' && nv && nv !== String(before.akhil_points || '').trim()) {
      const { data: pg } = await supabase.from('users').select('username').ilike('full_name', 'POONAM GUPTA').limit(1)
      const pgUser = pg?.[0]?.username
      if (pgUser) {
        const today = new Date().toISOString().split('T')[0]
        await supabase.from('reminders').insert({ order_id: parseInt(req.params.id), order_no: before.order_no || '', client: before.client || '', description: `AKHIL PAYMENT REMARKS UPDATED: ${nv}`, date: today, visible_to: [pgUser], assigned_to: pgUser, created_by: req.user.username })
      }
    }
